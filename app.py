...
def request_swap(skill_id):
    ...
    # Skill that user wants to swap (from other user)
    skill = conn.execute('''
    ...
    # Fetch skills that I have offered (to show in dropdown)
    my_skills_offered = conn.execute('''
        SELECT * FROM skills_offered WHERE user_id = ?
    ''', (session['user_id'],)).fetchall()
    
    conn.close()
    
    return render_template('request_swap.html', skill=skill, my_skills_offered=my_skills_offered)
